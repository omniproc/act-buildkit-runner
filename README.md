...
Starting with the next release, the container runs as non-root user `runner` (uid 1001) instead of `root`. Workflows that depend on root privileges (e.g. `apt-get install`, writing to system directories) will need adjustment. See the [Kubernetes](#kubernetes) section for PersistentVolume considerations.
